<template lang="">
       ...
       <div class="col-lg-12 px-0">
                <div class="row mx-0">
                    ...
                    v-for="(item, index) in articles2.slice(45)" :key="index"
    ...
    data() {
      ...
      methods: {
          ...
          async getData2() {
            ...
            try {
              const response = await fetch(
                `https://api-epicnews404.azurewebsites.net/Articles/TopHeadlines?SiteId=1&CategoryId=6&Language=14&Page=3&PageSize=${pageSize}`
              );
              const data = await response.json();
              return data.items;
            } catch (error) {
