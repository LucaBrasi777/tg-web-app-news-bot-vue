<template lang="">
  ...
  <div class="col-lg-12 px-0">
      <div class="row mx-0">
          ...
          v-for="(item,index)  in articles.slice(6)" :key="index">
              <div class="position-relative overflow-hidden" style="height: 250px;">
                 
                <img class="img-fluid w-100 h-100" v-if="item.urlToImage" :src="item.urlToImage" style="object-fit: cover;">
    <img class="img-fluid w-100 h-100" v-else src="https://www.ripplesnigeria.com/wp-content/uploads/2020/03/Latest-tech-news.jpg" style="object-fit: cover;">
...
async getData2() {
  ...
  try {
    const response = await fetch(
      `https://api-epicnews404.azurewebsites.net/Articles/TopHeadlines?SiteId=1&CategoryId=8&Language=14&WithImageOnly=true&Page=2&PageSize=${pageSize}`
    );
    const data = await response.json();
    return data.items;
  } catch (error) {
